...
exports.validateCreateReminder = ({ task_id, remind_at }) => {
  ...
  if (isNaN(remindAtDate.getTime())) throw new appError('remind_at must be a valid date', 400);
  if (remindAtDate.getTime() < now.getTime()) throw new appError('remind_at cannot be in the past', 400);
};
